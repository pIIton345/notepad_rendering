...
def parse_args():
    p = argparse.ArgumentParser(description="Fast Mixed Camera/Video/Doom -> ASCII")
    p.add_argument("--input", help="Video file path or camera index (0,1,...)")
    p.add_argument("--wad", help="Path to .wad file (Enables Doom Mode)")
    p.add_argument("--output", required=True, help="Output text file path")
    p.add_argument("--width", type=int, default=120, help="ASCII width in characters")
    p.add_argument("--workers", type=int, default=4, help="Number of worker threads (video only)")
    p.add_argument("--buffer", type=int, default=8, help="Max frames to buffer")
    ...
    p.add_argument("--auto", action="store_true", help="Adaptive brightness")
    p.add_argument("--auto-gamma", type=float, default=1.0, help="Gamma for auto brightness")
    p.add_argument("--aspect", type=float, default=0.5, help="Character aspect ratio")
    return p.parse_args()
